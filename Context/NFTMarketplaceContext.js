...
export const NFTMarketplaceProvider = ({ children }) => {
  const titleData = "NFT های خود را پیدا کنید، بفروشید و کسب درآمد کنید.";
  ...
  const checkIfWalletConnected = async () => {
    try {
      if (!window.ethereum)
        return setOpenError(true), setError("کیف پول خود را متصل کنید");
  ...
  const uploadToIPFS = async (file) => {
    try {
      ...
      setError("متاسفیم ارتباط با سرور برقرار نشد");
      setOpenError(true);
    }
  };
  ...
  const createNFT = async (name, price, image, description, router) => {
    if (!name || !description || !price || !image)
      return setError("اطلاعات تکمیل نمی باشد."), setOpenError(true);
    ...
    try {
      ...
      setError("خطایی پیش آمده است، لطفا دوباره امتحان کنید.");
      setOpenError(true);
    }
  };
  ...
  const createSale = async (url, formInputPrice, isReselling, id) => {
    try {
      ...
      setError("خطایی در هنگام فروش پیش آمده اتس لطفا مجدد تلاش کنید.");
      setOpenError(true);
      console.log(error);
    }
  };
  ...
  const buyNFT = async (nft) => {
    try {
      ...
      setError("خطایی در هنگام خرید پیش آمده است لطفا مجدد تلاش کنید.");
      setOpenError(true);
    }
  };
